...
func (s *Summary) PrintWarnings() {
	...
	if totalCount.HTTPRequests == 0 && totalCount.HTTPResponses == 0 {
		...
		if env.InDocker() && env.HasDockerInternalHostAddress() {
			printer.Stderr.Infof("If you're using macOS and your service is not running in a Docker container, try using the native Postman Live Collections Agent with `brew install akita-cli`.  See docs.akita.software/docs/macos-local for details.\n")
		}
